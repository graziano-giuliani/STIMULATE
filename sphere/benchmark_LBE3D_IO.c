...
void init_sphere( double * rho, int * grid_global_dimensions, int step ){

  int i, j, k;
  double x0 = ( grid_global_dimensions[ 0 ] / 2 + step ) % grid_global_dimensions[ 0 ]; 
  double y0 = grid_global_dimensions[ 1 ] / 2.0, z0 = grid_global_dimensions[ 2 ] / 2.0; 
  for( i = 1; i <= NX; i++ ){
    for( j = 1; j <= NY; j++ ){
      for(k = 1; k <= NZ; k++ ){
	
	size_t global_i = i + ( my_coords[ 0 ] * NX );
	size_t global_j = j + ( my_coords[ 1 ] * NY );
	size_t global_k = k + ( my_coords[ 2 ] * NZ );	
	...
	if( ( ( delta_x ) * (delta_x ) + ( delta_y ) * ( delta_y ) + ( delta_z ) * ( delta_z ) ) <= ( grid_global_dimensions[ 0 ] * 2.0 ) ){
	  rho[ idx ] = 0.8;
	} else{
	  rho[ idx ] = 0.2;
	}
...
int main (int argc, char * argv[])
{
  ...
  if( mype == 0 ){
    ...
    fprintf( stdout, "\n\tTotol bytes = %ld, Total bytes per process = %ld\n\n", ( (size_t) grid_global_dimensions[ 0 ] * grid_global_dimensions[ 1 ] * grid_global_dimensions[ 2 ] ) * sizeof( double ), NX * NY * NZ * sizeof( double ) );
  }

  for( istep = 0; istep < N_STEPS; istep++ ){
    
    init_sphere( rho, grid_global_dimensions, istep );
